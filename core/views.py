...
def employee_dashboard(request):
    ...
    closed_bookings = Booking.objects.filter(status__in=['Rejected', 'Cancelled', 'Completed']).order_by('-created_at')[:20]
    rooms = Room.objects.all()
    gcash_accounts = GCashAccount.objects.all().order_by('-created_at')
...
def confirm_booking(request, booking_id):
    ...
    if booking.status == 'Completed' or booking.is_past_stay:
        messages.warning(request, "This booking has already passed and is marked completed.")
        return redirect('employee_dashboard')
    if booking.status != 'Pending':
        messages.warning(request, "Only pending bookings can be confirmed.")
        return redirect('employee_dashboard')
...
def reject_booking(request, booking_id):
    ...
    if booking.status == 'Completed':
        messages.warning(request, "Completed bookings can no longer be rejected.")
        return redirect('employee_dashboard')
    if booking.status != 'Pending':
...
def edit_booking(request, booking_id):
    ...
    if booking.status in ['Rejected', 'Cancelled', 'Completed']:
        messages.warning(request, "Closed bookings can no longer be modified.")
        return redirect(booking_return_url(request.user))
...
def cancel_booking(request, booking_id):
    ...
    if request.method == 'POST':
        if booking.status in ['Rejected', 'Cancelled', 'Completed']:
            ...
            messages.warning(request, "This stay date has already passed, so the booking is marked completed instead of cancelled.")
        else:
